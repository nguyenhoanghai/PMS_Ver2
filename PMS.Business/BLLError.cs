    ...
    public class BLLError
    {
        public static List<ErrorModel> GetAll()
        {
            try
            {
                ...
                return db.Errors.Where(x => !x.IsDeleted && !x.GroupError.IsDeleted && (x.P_Phase==null || !x.P_Phase.IsDeleted)).Select(x => new ErrorModel()
                {
                    Id = x.Id,
                    Name = x.Name,
                    Code = x.Code,
        ...
        public static Error CheckExists(Error obj)
        {
            var db = new PMSEntities();
            return db.Errors.FirstOrDefault(x => !x.IsDeleted && x.Id != obj.Id && ((x.Code == obj.Code && obj.PhaseId ==null) || ( x.PhaseId == obj.PhaseId && x.Code == obj.Code)));
        }
