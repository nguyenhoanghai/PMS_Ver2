...
namespace QuanLyNangSuat
{
    public partial class FrmBTP_HCStruct : Form
    {
        ...
        private void FrmBTP_HCStruct_Load(object sender, EventArgs e)
        {
            ...
            var items = new List<ModelSelectItem>();
            items.Add(new ModelSelectItem() { Id = (int)ePhaseType.BTP_HC, Name = "Công đoạn BTP hoàn chỉnh" });
            items.Add(new ModelSelectItem() { Id = (int)ePhaseType.HOANTAT, Name = "Công đoạn Hoàn tất" });
            items.Add(new ModelSelectItem() { Id = (int)ePhaseType.LOI, Name = "Công đoạn Lỗi" });
        ...
        {
            var obj = (ModelSelectItem)cbType.SelectedItem;
            if(obj!= null)
            {
                phaseType = obj.Id;
                LoadData();
