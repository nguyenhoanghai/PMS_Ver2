...
namespace DuAn03_HaiDang
{
    public partial class frmPhaHangChoChuyen : Form
    {
        ...
        private void LoadColors()
        {
            ...
            cbColor.Text = "(-- Chọn Màu --)";
            if (!string.IsNullOrEmpty(AccountSuccess.strListChuyenId))
            {
                var items = BLLColor.Instance.GetSelects( );
                if (items != null && items.Count > 0)
                {
                    cbColor.DataSource = items;
        ...
        {
            cbSize.DataSource = null;
            cbSize.Text = "(-- Chọn Size --)";
            if (!string.IsNullOrEmpty(AccountSuccess.strListChuyenId))
            {
                var items = BLLSize.Instance.GetSelects();
                if (items != null && items.Count > 0)
                {
                    cbSize.DataSource = items;
        ...
        private void gridView1_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            try
            {
                ...
                cbSize.Text = (gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "SizeName") != null ? 
                        gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "SizeName").ToString(): "(-- Chọn Size --)");
                cbColor.Text = (gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "ColorName")!= null ?
                    gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "ColorName").ToString(): "(-- Chọn Màu --)");
        ...
        private void reloadColor_Click(object sender, EventArgs e)
        {
            LoadColors();
        }

        private void reloadSize_Click(object sender, EventArgs e)
        {
            LoadSizes();
        }
    }
